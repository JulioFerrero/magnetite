...
enum Look: String, CaseIterable, Identifiable {
    ...
    var panelSize: NSSize {
        NSSize(width: Theme.windowSize.width + margin.left + margin.right, height: Theme.windowSize.height + margin.top + margin.bottom)
    }
}
...
enum Theme {
    ...
    static let glassTint = dynamic(NSColor(white: 0, alpha: 0.40), NSColor(white: 1, alpha: 0.40))
    static let clearGlassTint = dynamic(NSColor(white: 0, alpha: 0.55), NSColor(white: 1, alpha: 0.62))
    static let solidBackground = dynamic(NSColor(white: 0x26 / 255.0, alpha: 1), .white)
    static let solidPill = dynamic(NSColor(white: 0x33 / 255.0, alpha: 1), NSColor(white: 0xF2 / 255.0, alpha: 1))
    static let solidBorder = dynamic(NSColor(white: 1, alpha: 0.10), NSColor(white: 0, alpha: 0.10))
    static let windowBorder = dynamic(fg(0.10), .clear)
    ...
    static let accessoryGap = 16.0, runningDotSize = 3.0, smallButtonSize = 22.0
    static let sectionHeight = 32.0, sectionLabelCenterY = 19.0, clearButtonGap = 6.0
    ...
    private static func dynamic(_ dark: NSColor, _ light: NSColor) -> NSColor {
